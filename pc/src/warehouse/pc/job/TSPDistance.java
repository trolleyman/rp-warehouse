...
public class TSPDistance {
	...
	public int getDistance(Job job, int x, int y){
		...
		for(int i=0; i<items.size(); i++){
			dToItem[i] = (routeFinder.findRoute(new Junction(x, y), items.get(i).getItem().getJunction(), Direction.X_POS).getDirectionList().size());
		}
		...
		for(int i=0; i<items.size(); i++){
			for(int j=0; j<items.size(); j++){
				itemToItem[i][j] = (routeFinder.findRoute(items.get(i).getItem().getJunction(), items.get(j).getItem().getJunction(), Direction.X_POS).getDirectionList().size());
			}
		}
		...
		for(int i=0; i<items.size(); i++){
			...
			for(int j=0; j<dropLocations.size(); j++){
				int dist = routeFinder.findRoute(items.get(i).getItem().getJunction(), dropLocations.get(j), Direction.X_POS).getDirectionList().size();
				if(dist < temp){
					temp = dist;
				}
			}
