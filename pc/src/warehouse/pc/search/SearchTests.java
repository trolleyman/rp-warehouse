 ...
 * 
 * Not getting caught out by unique situations (e.g. goal = start) Producing
 * paths that actually lead to the goal Producing optimal paths
 * 
 * @author George Kaye
 *
 */
...
public class SearchTests {
	...
	Command r = new Command(CommandType.RIGHT);
	Command l = new Command(CommandType.LEFT);
	Command f = new Command(CommandType.FORWARD);
	Command b = new Command(CommandType.BACKWARD);
	...
	private LinkedList<Command> testRoute(RouteFinder finder, Map map, int startx, int starty, int goalx, int goaly,
			Direction direction) {
