...
public class MultiRouteFinder {
	...
	private LinkedList<Command> getActualDirections(ArrayList<Direction> oldList, Direction direction) {
		...
		for (int i = 0; i < oldList.size(); i++) {
			...
			CommandType bearing = null;

			switch (direction) {
			case Y_POS:
				switch (currentDirection) {
				case Y_POS:
					bearing = CommandType.FORWARD;
					break;
				case Y_NEG:
					bearing = CommandType.BACKWARD;
					break;
				case X_POS:
					bearing = CommandType.RIGHT;
					break;
				case X_NEG:
					bearing = CommandType.LEFT;
					break;
			...
			newList.add(new Command(bearing));
